...
import BookCards from '../Common/BookCards.vue';
...
const getCategory = () => {
    fetch('https://qlapi.stesting.uz/api/v1/publication/category/')
        .then((res) => res.json())
        .then((data) => {
...
const getBook = (slug = '', pageNum = 1) => {
    fetch(`https://qlapi.stesting.uz/api/v1/publication/?page=${pageNum}&category__slug=${slug}`)
        .then((res) => res.json())
        .then((data) => {
            bookCard.value = data.results;
...
onMounted(() => {
    ...
    getBook(selectedSlug, page.value);
});
...
const getSlug = (selectedTitle) => {
    if (selectedTitle === 'Barchasi') {
        ...
        const selectedItem = category.value.find(
